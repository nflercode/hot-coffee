...
const JoinPage = () => {
    ...
    useEffect(() => {
        async function getTable() {
            const { data } = await tableService.getTableByInvitationToken(
                invitationToken
            );
    ...
    function handleJoinTableButtonClick() {
        async function joinTable() {
            const { data } = await tableService.joinTable(
                tableState.invitationToken,
                playerName
            );
    ...
    const annanOrAndra =
        (tableState.players || []).length === 1 ? "annan" : "andra";
    return (
        <main className="join-page-main">
            ...
            <div>
                <span>
                    {(tableState.players || []).length} {annanOrAndra} sitter
                    vid det här bordet
                </span>
            </div>
            <div>
            ...
            <div className="join-page-done-btn">
                <Button block onClick={handleJoinTableButtonClick}>
                    Anslut till bord
                </Button>
            </div>
        </main>
